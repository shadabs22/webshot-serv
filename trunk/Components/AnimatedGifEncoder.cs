...
namespace Gif.Components
{
	...
	{
        internal int width; // image size
        internal int height;
        internal Color transparent = Color.Empty; // transparent color if given
        internal int transIndex; // transparent index in color table
        internal int repeat = -1; // no repeat
        internal int delay = 0; // frame delay (hundredths)
        internal bool started = false; // ready to output frames
		//	protected BinaryWriter bw;
        internal Stream fs;

        internal Image image; // current frame
        internal byte[] pixels; // BGR byte array from frame
        internal byte[] indexedPixels; // converted frame indexed to palette
        internal int colorDepth; // number of bit planes
		internal byte[] colorTab; // RGB palette
        internal bool[] usedEntry = new bool[256]; // active palette entries
        internal int palSize = 7; // color table size (bits-1)
        internal int dispose = -1; // disposal code (-1 = use default)
        internal bool closeStream = false; // close stream when finished
        internal bool firstFrame = true;
        internal bool sizeSet = false; // if false, get size from first frame
        internal int sample = 10; // default sample interval for quantizer
		 ...
		 */
		public bool AddFrame(Image im, int thumbwidth, int thumbheight) 
		{
			if ((im == null) || !started) 
			{
				return false;
			...
			try 
			{
				...
				if (firstFrame) 
				{
                    WriteLSD(thumbwidth, thumbheight); // logical screen descriptior
                ...
                WriteImageDesc(thumbwidth, thumbheight); // image descriptor
				if (!firstFrame) 
				{
...
//			}
		...
		protected void GetImagePixels() 
		{
			...
			*/
            ...
            FastBitmap FBitmap = new FastBitmap(tempBitmap);

			for (int th = 0; th < image.Height; th++)
			{
				for (int tw = 0; tw < image.Width; tw++)
				{
                    Color color = FBitmap.GetPixel(tw, th);
		 ...
		 */
        protected void WriteImageDesc(int thumbwidth, int thumbheight)
		{
			fs.WriteByte(0x2c); // image separator
            WriteShort((thumbwidth / 2) - (width / 2)); // image position x,y = 0,0
            WriteShort((thumbheight / 2) - (height / 2));
        ...
        protected void WriteLSD(int thumbwidth, int thumbheight)  
		{
			// logical screen size
            WriteShort(thumbwidth);
            WriteShort(thumbheight);
			// packed fields
